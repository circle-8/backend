...
public class ZonaListTest {
	...
	void testListOk() {
		RestAssured.given()
			...
			.body("data[1].tipoResiduo",hasSize(2))
			.body("data[1].tipoResiduo[0].id", equalTo(3))
			.body("data[1].tipoResiduo[0].nombre", equalTo("Pilas"))
			.body("data[1].tipoResiduo[1].id", equalTo(4))
			.body("data[1].tipoResiduo[1].nombre", equalTo("Carton"))
		;
	}
